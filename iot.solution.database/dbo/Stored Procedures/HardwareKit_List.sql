...
BEGIN
    ...
    BEGIN TRY            
		...
		;WITH Devices
		AS 
		(	SELECT [kitGuid],COUNT([guid]) totalDevices FROM [dbo].[KitDevice] WHERE [isActive] = 1 AND [isDeleted] = 0 GROUP BY [kitGuid]
		)
		UPDATE T
		SET [deviceCount] = ISNULL(KT.totalDevices,0)
		FROM #temp_hardwareKit T
		LEFT JOIN Devices KT (NOLOCK) ON T.[guid] = KT.[kitGuid] 
