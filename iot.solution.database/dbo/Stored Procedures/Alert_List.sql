   ...
   BEGIN TRY
		...
		SET @Sql = '	SELECT
						...
						(	SELECT
							...
							INNER JOIN dbo.[Device] E WITH (NOLOCK) ON I.[deviceGuid] = E.[guid] AND E.[isDeleted] = 0
							JOIN dbo.[GreenHouse] G WITH (NOLOCK) ON I.[entityGuid] = G.[guid] AND G.[isDeleted] = 0
							WHERE I.[companyGuid] = ISNULL(@companyGuid,I.[companyGuid])
								AND I.[entityGuid] = ISNULL(@entityGuid,I.[entityGuid])
