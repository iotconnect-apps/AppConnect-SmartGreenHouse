	...
	FROM @UserXml.nodes('/items/item') a(b)
		...
		BEGIN
			...
			INSERT INTO dbo.[User]
				([guid],[email], [firstName], [lastName], [greenhouseGuid], [companyGuid], [isActive], [isDeleted],[createdDate],[timeZoneGuid], [contactNo], [roleGuid])
		...
		BEGIN
			...
			SET  [firstName] = CASE WHEN te.hasFirstName = 1 THEN te.firstName ELSE d.firstName END
				...
				,[greenhouseGuid] = CASE WHEN te.hasEntityguid = 1 THEN te.entityguid ELSE d.[greenhouseGuid] END				
