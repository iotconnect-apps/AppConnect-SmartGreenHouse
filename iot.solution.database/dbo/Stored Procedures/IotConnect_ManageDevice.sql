...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			INSERT INTO dbo.[device]([guid], [uniqueId], [name], [companyGuid], [greenHouseGuid], [isActive], [isDeleted], [templateGuid], [isProvisioned], [tag], [parentDeviceGuid],[createdDate],[note])
			SELECT DISTINCT [guid], uniqueId, ISNULL([name],''), @companyGuid, entityGuid, isActive, isDeleted, deviceTemplateGuid, IsNULL(isAcquired,0), LOWER(tag), parentDeviceGuid, GETUTCDATE(),ISNULL([note],'')
			FROM #tempDevice td
		...
		BEGIN
			UPDATE d
				SET  [uniqueId]			= CASE WHEN td.hasUniqueId = 1 THEN td.uniqueId ELSE d.[uniqueId] END
					,[greenHouseGuid]	= CASE WHEN td.hasentityGuid = 1 THEN td.entityGuid ELSE d.greenHouseGuid END
					,[templateGuid]		= CASE WHEN td.hasdevicetemplateGuid = 1 THEN td.deviceTemplateGuid ELSE d.templateGuid END
					,[isProvisioned]	= CASE WHEN td.hasisAcquired = 1 THEN td.isAcquired ELSE d.isProvisioned END
