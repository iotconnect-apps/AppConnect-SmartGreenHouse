...
001	sgh-1 05-12-2019 [Nishit Khakhi]	Added Initial Version to Delete GreenHouse
*******************************************************************/
CREATE PROCEDURE [dbo].[GreenHouse_Delete]
(	 
...
BEGIN
	...
	BEGIN
        ...
        SELECT @Param = 
        (
            SELECT 'GreenHouse_Delete' AS '@procName'             
    ...
    BEGIN TRY            
		...
		IF NOT EXISTS (SELECT TOP 1 1 FROM [dbo].[GreenHouse] (NOLOCK) WHERE [companyGuid] = @companyguid AND [guid] = @guid AND [isdeleted] = 0)
		BEGIN
			SET @output = -3
