	...
	FROM @DeviceTemplateXml.nodes('/items/item') a(b)
		...
		BEGIN TRAN
			...
			BEGIN
				...
				(
					SELECT [guid] FROM [dbo].[KitType] k (NOLOCK)
				)

				INSERT INTO [dbo].[KitType] ([guid], [code], [name], [isDeleted], [tag], [isActive])
				SELECT DISTINCT [guid], [code], [name], [isDeleted], LOWER([tag]), 1
