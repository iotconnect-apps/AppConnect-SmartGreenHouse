    ...
    public class DatabaseManager : IDatabaseManager
    {
        ...
        public void DeviceProcessMessage(MessageModel subscribeData)
        {
            try
            {
                ...
                //XML device with or without child device
                var xmlDeviceData = Convert.ToString(JsonConvert.DeserializeXNode(JsonConvert.SerializeObject(subscribeData.Data), "items"));
                XmlDocument xmlDoc = new XmlDocument();
                xmlDoc.LoadXml(xmlDeviceData);
                if (xmlDoc.ChildNodes.Count > 0)
                {
                    xmlDeviceData = xmlDoc.ChildNodes[0].InnerXml;
                }
                var baseDataAccess = new BaseDataAccess(_connectionString);
                var sqlParameters = new List<SqlParameter>
                {
